...
subsegment () {
    ...
    T2_scan=/home/ubuntu/volume/Tinception/subjects_raw/temp_T2/${subject_id}.nii.gz
    

    if [[ "$subject_id" == p* ]]; then
        # Subject starts with p: Use T1 version
        segmentHA_T1.sh "$subject_id"
    else
        # Other allowed subjects: Use T2 version
...
# main part
allowed_subjects=("cwax" " fsox" " mqkt" " nfru" " qxja" "rojk" "urhq" "vazl" "vywa" "wffn" "wpok")
...
for subj_dir in "$SUBJECTS_DIR"/*; do
	...
	#subsegment "$subject_id"
        ...
        # Check if subject_id is in the allowed list
        if [[ " ${allowed_subjects[@]} " =~ " ${subject_id} " ]] || [[ "$subject_id" == p* ]] || [[ "$subject_id" == MP* ]]; then
            subsegment "$subject_id"
        fi
    fi
done
