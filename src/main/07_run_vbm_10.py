...
## run TFCE
img_4d = load_img(gms_fname)
subset_idx = np.asarray(subset_idx).astype(int)
all_imgs = list(iter_img(img_4d))
imgs_subset = [all_imgs[i] for i in subset_idx]

print("working on ti > co ...")
out_ti_gt_co = non_parametric_inference(
                                second_level_input=imgs_subset,
                                design_matrix=design_matrix,
                                second_level_contrast=f"{group_col}",
                                mask=mask_img,
...
print("working on co > ti ...")
out_co_gt_ti = non_parametric_inference(
                                second_level_input=imgs_subset,
                                design_matrix=design_matrix,
                                second_level_contrast=f"-{group_col}",
                                mask=mask_img,
...
sel_images = [out_ti_gt_co, out_co_gt_ti]
cuts = [(-18, 6, -9), (1, 34, -15)]
titles = ["ti_gt_co", "co_gt_ti"]
for sel_image, cut, title in zip(sel_images, cuts, titles):
    ...
    fig = plotting.plot_stat_map(
                stat_map_img=sel_image["logp_max_tfce"],
                bg_img=img_bg,
                display_mode="ortho",
                **kwargs
                )
