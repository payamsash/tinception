...
for subject_path in "$tinspect_dir"/*; do
    if [ -d "$subject_path" ]; then
        ...
        hdr_file=$(find "$t1w_dir" -maxdepth 1 -type f -name '*.hdr' | head -n 1)
        fslchfiletype NIFTI $hdr_file "$tinspect_subjects_dir/$subject_id.nii"
    fi
done
...
cd $subjects_raw
nohup bash -c 'ls *.nii | parallel --jobs $n_threads recon-all -s {.} -i {} -all' &